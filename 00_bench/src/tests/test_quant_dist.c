...
void print_top_k_ranks(void *ref, uint32_t k)
{
    struct PageRankStats *stats = (struct PageRankStats * )ref;

    printf("*-----------------------------------------------------*\n");
    printf("Printing Top %d Ranks\n", k);
    printf("*-----------------------------------------------------*\n");
...
double get_avg_error_relative(void *cmp, void *ref)
{
    ...
    uint32_t cmp_size = cmp_stats->num_vertices;
    uint32_t *cmp_arr = cmp_stats->realRanks;
    
    uint32_t ref_size = ref_stats->num_vertices;
    uint32_t *ref_arr2 = ref_stats->realRanks;
    ...
    uint32_t *cmp_labels = (uint32_t *) my_malloc(cmp_size * sizeof(uint32_t));
    uint32_t *ref_labels = (uint32_t *) my_malloc(ref_size * sizeof(uint32_t));

    for(i = 0; i < ref_size; i++)
    {
        cmp_labels[cmp_arr[i]] = i+1;
        ref_labels[ref_arr[i]] = i+1;
    }


    for(i = 0 ; i < ref_size; i++)
    {
        error += fabs(ref_labels[i] - cmp_labels[i])/(double)ref_labels[i];
    }
...
int main (int argc, char **argv)
{
    ...
    FILE *fp = fopen("quant_rank_shift.out", "w+");
    ...
    for(uint32_t i = 0; i < 1; i++)
    {
        ...
        for(arguments.pushpull = 0 ; arguments.pushpull < 12; arguments.pushpull++)
        {
            ...
            if(ref_data != NULL && cmp_data != NULL)
            {
                ...
                fprintf(fp, "Avg Error: %lf\t", avg_error_float);
                printf("avg_error_float: %lf\t", avg_error_float);

                double avg_dist = get_avg_distortion(cmp_data, ref_data);
                fprintf(fp, "Avg Distortion: %lf\t", avg_dist);
                printf("Avg Distortion: %lf\t", avg_dist);
