...
//
// -----------------------------------------------------------------------------
// graph-unit-pagerank-kernel: cu_PageRank/CSR/PULL cu_sum_kernel_control for the
// FixedPoint, Quantized and FloatPoint precision contexts.
//
// Oracle: independent-pagerank-precision-v1.
//
// Precision policy verified here (authored from the arithmetic contract, not
// from the RTL):
//   * FixedPoint  : DATA_SIZE_READ = 8 bytes, DATA_SIZE_WRITE = 8 bytes.  The
//                   per-vertex result is the exact integer sum of the streamed
//                   in-neighbour contributions truncated to 64 bits.  No
//                   rounding, no saturation: overflow wraps modulo 2^64.
//   * Quantized   : DATA_SIZE_READ = 4 bytes zero extended into the 8 byte
//                   accumulator.  Same wrap policy, quantisation points are the
//                   32 bit read values.
//   * FloatPoint  : IEEE-754 binary32 sequential accumulation in stream order
//                   with round-to-nearest-even, one accumulator restart per
//                   vertex, and a 16 cycle drain before the result is latched.
//                   The licensed Quartus accumulator is replaced by an
//                   independent behavioural stand-in; bit exactness against the
//                   licensed IP stays release evidence.
// -----------------------------------------------------------------------------
...
////////////////////////////////////////////////////////////////////////////
	...
	task automatic bundle(input string reason);
		$display("FAILURE-BUNDLE pagerank_kernel reason=%s", reason);
	...
	task automatic fail(input string reason);
		bundle(reason);
		$error("pagerank_kernel mismatch %s", reason);
		$fatal(1);
	endtask
...
`ifdef KERNEL_FLOAT
		case (class_id)
			...
			CV_MIXED: return ((k % 2) == 0) ?
				...
				(32'h8000_0000 | fp32_from_ratio(k + 1, 7));      // negative contribution
...
`else
		case (class_id)
			...
			CV_MIXED: return ((k % 2) == 0) ?
				(DATA_SIZE_READ_BITS)'(64'hFFFF_FFFF_0000_0001) :
...
////////////////////////////////////////////////////////////////////////////
	...
	task automatic drive_vertex(input int fx, input int v, input int class_id);
		...
		for (k = 0; k < deg; k++) begin
			value = contribution(class_id, fx, v, k);
`ifdef KERNEL_FLOAT
			// the accumulator restarts on the first contribution, then adds each
			// following contribution in stream order with binary32 rounding
			acc = (k == 0) ? value : fp32_add(acc, value);
`else
			acc = acc + (DATA_SIZE_WRITE_BITS)'(value);
`endif
		end
...
`ifdef KERNEL_FLOAT
	...
	task automatic check_float_drain(output bit found, output int shortened_window);
		...
		if (defect_writes != 1) begin
			$display("DEFECT-SETUP pagerank_kernel writes=%0d expected=1", defect_writes);
			found = 1'b1;
		end else if (defect_data !== (DATA_SIZE_WRITE_BITS)'(value)) begin
			found = 1'b1;
			$display("  observed=%h expected=%h previous_result=%h", defect_data, value, previous);
		end
		defect_mode = 1'b0;
	endtask
`endif
	...
	initial begin
		...
		run_fixture(FX_SHARED_TRIANGLES, CV_SMALL, 0);
...
`ifdef KERNEL_FLOAT
		...
		cover_bin[17] = 1'b1;
		if (stream_values == 0)
			fail("float accumulator never received a contribution");
		cover_bin[18] = 1'b1;
`else
		cover_bin[17] = 1'b1;
		cover_bin[18] = 1'b1;
`endif
		...
		if (bins_hit != BIN_COUNT) begin
			for (int b = 0; b < BIN_COUNT; b++)
				if (!cover_bin[b])
					$display("MISSING-BIN pagerank_kernel bin=%0d", b);
			fail($sformatf("functional bins %0d/%0d", bins_hit, BIN_COUNT));
		end

		$display("PASS pagerank_kernel_unit scenarios=%0d vectors=%0d writes=%0d bins=%0d masks=%0d",
