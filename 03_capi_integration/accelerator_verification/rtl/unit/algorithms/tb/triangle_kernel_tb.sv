...
//
// -----------------------------------------------------------------------------
// graph-unit-triangle-kernel: cu_TriangleCount/CSR/BinaryIntersection/Binary
// cu_update_kernel_control, the binary intersection counting kernel.
//
// Oracle: independent-triangle-count-v1.
//
// Counting policy verified here (authored from the host binary intersection
// policy, not from the RTL): the engine streams one sorted-neighbour comparison
// per element, payload.data carrying the probed neighbour and payload.weight the
// candidate reached by the binary search.  The kernel counts one triangle per
// equality, publishes the per-vertex count once inverse_out_degree elements have
// been consumed, publishes nothing for an empty slice, and wraps modulo
// 2^DATA_SIZE_WRITE_BITS.  The expected count is the independent per-vertex
// triangle count of the fixture.
// -----------------------------------------------------------------------------
...
////////////////////////////////////////////////////////////////////////////
	...
	task automatic bundle(input string reason);
		$display("FAILURE-BUNDLE triangle_kernel reason=%s", reason);
	...
	task automatic fail(input string reason);
		bundle(reason);
		$error("triangle_kernel mismatch %s", reason);
		$fatal(1);
	endtask
...
////////////////////////////////////////////////////////////////////////////
	...
	// One sorted-neighbour comparison element: the probed neighbour and the
	// candidate the binary search reached in the compared adjacency list.  A
	// pair that closes a triangle produces an equal candidate; a pair that does
	// not produces a definite miss, derived independently from the fixture.
	function automatic logic [0:(DATA_SIZE_READ_BITS-1)] probe_value(
			input int class_id, input int fx, input int v, input int k);
		int b;
		b = pair_b(fx, v, k);
		case (class_id)
			CV_MAX  : return {DATA_SIZE_READ_BITS{1'b1}} - (DATA_SIZE_READ_BITS)'(b);
			default : return (DATA_SIZE_READ_BITS)'(b);
		endcase
	endfunction

	function automatic logic [0:(EDGE_WEIGHT_SIZE_BITS-1)] candidate_value(
			input int class_id, input int fx, input int v, input int k);
		int a;
		int b;
		a = pair_a(fx, v, k);
		b = pair_b(fx, v, k);
		if (adjacent(fx, a, b))
			return (EDGE_WEIGHT_SIZE_BITS)'(probe_value(class_id, fx, v, k));
		// a definite miss: the same width, one bit apart from the probe
		return (EDGE_WEIGHT_SIZE_BITS)'(probe_value(class_id, fx, v, k)) ^
			{1'b1, {(EDGE_WEIGHT_SIZE_BITS-1){1'b0}}};
	endfunction
...
////////////////////////////////////////////////////////////////////////////
	...
	task automatic drive_vertex(input int fx, input int v, input int class_id);
		...
		// independent golden: the per-vertex triangle count of the fixture
		expected = (DATA_SIZE_WRITE_BITS)'(triangle_count_vertex(fx, v));
		for (k = 0; k < elements; k++) begin
			if (adjacent(fx, pair_a(fx, v, k), pair_b(fx, v, k)))
				cover_bin[3] = 1'b1;
			else
				cover_bin[4] = 1'b1;
		end
		...
		data_buffer_status.empty = (elements == 0);
		for (k = 0; k < elements; k++) begin
			stall_gap(DOMAIN_EDGE_READ);
			edge_data.valid           = 1'b1;
			edge_data.payload.data    = probe_value    (class_id, fx, v, k);
			edge_data.payload.weight  = candidate_value(class_id, fx, v, k);
			edge_data.payload.cu_id_x = CU_X;
			edge_data.payload.cu_id_y = CU_Y;
			vectors                   = vectors + 1;
			@(posedge clock);
			edge_data.valid          = 1'b0;
			data_buffer_status.empty = (k == elements - 1);
		end
		data_buffer_status.empty = 1'b1;

		repeat (ACC_DRAIN + 8) @(posedge clock);
	...
	// The triangle count increments by one per matching comparison, so the carry
	// chain of the wide result is walked by a long run of matching elements.
	task automatic census_carry_accumulation(input int unsigned elements);
		...
		for (int unsigned k = 0; k < elements; k++) begin
			data_buffer_status.empty = 1'b0;
			edge_data.valid          = 1'b1;
			edge_data.payload.data   = (DATA_SIZE_READ_BITS)'(k);
			edge_data.payload.weight = (EDGE_WEIGHT_SIZE_BITS)'(k);
			@(posedge clock);
			edge_data.valid = 1'b0;
		end
		data_buffer_status.empty = 1'b1;
	...
	task automatic run_fixture(input int fx, input int class_id, input int mask);
		...
		// 5: nominal identifiers, 6: wide identifier boundary
		cover_bin[(class_id == CV_MAX) ? 6 : 5] = 1'b1;
		if (mask != 0)
			cover_bin[8 + (mask % 4)] = 1'b1;
		if (triangle_count_total(fx) > 0)
			cover_bin[7] = 1'b1;
	endtask
	...
	initial begin
		...
		run_fixture(FX_SINGLE_VERTEX   , CV_SMALL, 0); // no neighbour pair at all
		run_fixture(FX_CHAIN           , CV_SMALL, 0); // pairs that never close
		run_fixture(FX_STAR            , CV_SMALL, 0); // high degree, zero triangles
		...
		run_fixture(FX_SINK            , CV_SMALL, 0);
		run_fixture(FX_TRIANGLE        , CV_SMALL, 0); // smallest positive count
		run_fixture(FX_SHARED_TRIANGLES, CV_SMALL, 0); // shared edge normalisation
		run_fixture(FX_K4              , CV_SMALL, 0); // dense intersection
		run_fixture(FX_K4              , CV_MAX  , 0); // wide identifier boundary

		// iteration: repeat the same fixture with a second contribution class
		run_fixture(FX_TRIANGLE, CV_MAX, 0);
		cover_bin[13] = 1'b1;

		for (mask_index = 0; mask_index < MASK_COUNT; mask_index++)
			run_fixture(FX_K4, CV_SMALL, mask_index);
		...
		@(posedge clock);
		edge_data.valid        = 1'b1;
		edge_data.payload.data   = probe_value    (CV_SMALL, FX_K4, 0, 0);
		edge_data.payload.weight = candidate_value(CV_SMALL, FX_K4, 0, 0);
		@(posedge clock);
		apply_reset();
		completions_expected = 0;
		...
		run_fixture(FX_TRIANGLE, CV_SMALL, 0);
		cover_bin[15] = 1'b1;
		...
		if (bins_hit != BIN_COUNT) begin
			for (int b = 0; b < BIN_COUNT; b++)
				if (!cover_bin[b])
					$display("MISSING-BIN triangle_kernel bin=%0d", b);
			fail($sformatf("functional bins %0d/%0d", bins_hit, BIN_COUNT));
		end

		$display("PASS triangle_kernel_unit scenarios=%0d vectors=%0d writes=%0d bins=%0d masks=%0d",
