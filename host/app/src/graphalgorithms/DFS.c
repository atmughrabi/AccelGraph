   ...
   for(i=0 ; i < P ; i++){
	...
	while(!isEmptyArrayQueue(sharedFrontierQueue)){ // start while 
		...
		if(mf > (mu/alpha)){
			...
			Start(timer_inner);
			mu -= mf;		
			mf = topDownStepDFSGraphCSR(graph, sharedFrontierQueue,localFrontierQueues);
...
void depthFirstSearchUsingBitmapsGraphCSR(__u32 source, struct GraphCSR* graph){
    ...
    while (sharedFrontierQueue->q_bitmap->numSetBits){
		...
		if(mf > (mu/alpha)){
			...
			Start(timer_inner);
			nf_prev = nf;
			nf = bottomUpStepDFSGraphCSR(graph,sharedFrontierQueue->q_bitmap,sharedFrontierQueue->q_bitmap_next);
			...
			Start(timer_inner);
			mf = topDownStepDFSUsingBitmapsGraphCSR(graph, sharedFrontierQueue);
...
void depthFirstSearchGraphGrid(__u32 source, struct GraphGrid* graph){
 ...
 //   for(i=0 ; i < P*P ; i++){
	...
	while(!isEmptyArrayQueue(sharedFrontierQueue)){ // start while 
		 ...
		 Start(timer_iteration);
			depthFirstSearchStreamEdgesGraphGrid(graph, sharedFrontierQueue, localFrontierQueues);
		 Stop(timer_iteration);
			...
			slideWindowArrayQueue(sharedFrontierQueue);
			arrayQueueGenerateBitmap(sharedFrontierQueue);
			depthFirstSearchSetActivePartitions(graph,sharedFrontierQueue);
    	...
    	for (i = 0; i < totalPartitions; ++i){
           		if(getBit(graph->grid->activePartitionsMap,i)){
           			...
           			// {
	            		...
	            		depthFirstSearchPartitionGraphGrid(graph,&(graph->grid->partitions[i]),sharedFrontierQueue,localFrontierQueue);
           				flushArrayQueueToShared(localFrontierQueue,sharedFrontierQueue);
           			// }
...
void depthFirstSearchGraphGridBitmap(__u32 source, struct GraphGrid* graph){
	...
	depthFirstSearchSetActivePartitionsBitmap(graph, FrontierBitmapCurr);
	...
	while(processed_nodes){ // start while 
		...
		Start(timer_iteration);
			depthFirstSearchStreamEdgesGraphGridBitmap(graph, FrontierBitmapCurr, FrontierBitmapNext);
		Stop(timer_iteration);
    ...
    {	
    	...
    	for (i = 0; i < totalPartitions; ++i){
           		if(getBit(graph->grid->activePartitionsMap,i) && graph->grid->partitions[i].num_edges){
	            		depthFirstSearchPartitionGraphGridBitmap(graph,&(graph->grid->partitions[i]),FrontierBitmapCurr,FrontierBitmapNext);
        	} 
        }
    }
}
	...
	while(!isEmptyArrayQueue(sharedFrontierQueue)){ // start while 
		...
		if(mf > (mu/alpha)){
			...
			do{
				Start(timer_inner);
				nf_prev = nf;
				nf = bottomUpStepDFSGraphAdjArrayList(graph,bitmapCurr,bitmapNext);
				swapBitmaps(&bitmapCurr, &bitmapNext);
				clearBitmap(bitmapNext);
				Stop(timer_inner);
			...
			Start(timer_inner);
			mu -= mf;		
			mf = topDownStepDFSGraphAdjArrayList(graph, sharedFrontierQueue,localFrontierQueues);
			slideWindowArrayQueue(sharedFrontierQueue);
			Stop(timer_inner);
			//stats collection
			inner_time +=  Seconds(timer_inner);
	...
	while(!isEmptyArrayQueue(sharedFrontierQueue)){ // start while 
		...
		if(mf > (mu/alpha)){
			...
			do{
				Start(timer_inner);
				nf_prev = nf;
				nf = bottomUpStepDFSGraphAdjLinkedList(graph,bitmapCurr,bitmapNext);
				swapBitmaps(&bitmapCurr, &bitmapNext);
				clearBitmap(bitmapNext);
				Stop(timer_inner);
			...
			Start(timer_inner);
			mu -= mf;		
			mf = topDownStepDFSGraphAdjLinkedList(graph, sharedFrontierQueue,localFrontierQueues);
			slideWindowArrayQueue(sharedFrontierQueue);
			Stop(timer_inner);
			//stats collection
			inner_time +=  Seconds(timer_inner);
